...
(() => {
  ...
  var civitai_base_models_default = {
    $comment: "Generated by scripts/fetch-civitai-basemodels.mjs - do not edit by hand. Consumed by frontend/civitaiFolders.ts; rebuild the bundle (npm run build) after updating.",
  ...
  // frontend/civitaiFolders.ts
  var KNOWN_ARCHITECTURES = civitai_base_models_default.baseModels;
  ...
  var folderForModel = (mappings2, modelType, baseModel) => {
    if (!baseModel) {
      return null;
    }
    ...
    for (const mapping of mappings2) {
      ...
      const folder = modelType === "Stable-Diffusion" ? mapping.checkpointFolder : modelType === "LoRA" ? mapping.loraFolder : "";
      if (folder) {
        return folder;
      }
    }
  ...
  var patchDownloader = (downloader, getMappings) => {
  ...
  var mappings = [];
  var started = false;
  var setCivitaiFolderMappings = (next) => {
    mappings = next;
  };
  var init = () => {
    if (started) {
    ...
    started = true;
    if (typeof modelDownloader === "undefined" || typeof modelDownloader?.getCivitaiMetadata !== "function") {
      return;
    }
  ...
  var renderCivitaiArchPicker = (selected, options) => {
  ...
  var started3 = false;
  var initCivitaiArchPickers = (root) => {
    if (started3) {
      return;
    }
    started3 = true;
  ...
  var renderCivitaiSelect = (className, placeholder, options, value) => {
  ...
  var renderCivitaiMappingRow = (mapping, options) => `
  ...
  var renderCivitaiMappingRows = (mappings2, options) => mappings2.map((m) => renderCivitaiMappingRow(m, options)).join("");
  var getCivitaiRowOptions = () => {
  ...
  var readCivitaiMappings = (root) => normalizeMappings(
    Array.from(root.querySelectorAll("[data-wtd-civitai-row]")).map(
      (row) => ({
        architectures: Array.from(
          row.querySelector(".wtd-civitai-arch")?.selectedOptions ?? []
        ).map((option) => option.value),
        checkpointFolder: row.querySelector(
          ".wtd-civitai-checkpoint"
        )?.value ?? "",
        loraFolder: row.querySelector(".wtd-civitai-lora")?.value ?? ""
      })
    )
  );
  ...
  var applyCivitaiMappings = (mappings2) => {
    civitaiFolderMappings = mappings2;
    setCivitaiFolderMappings(mappings2);
  ...
  var init4 = () => {
    ...
    document.getElementById("whattheduck-civitai-add")?.addEventListener("click", () => {
      document.getElementById("whattheduck-civitai-mappings")?.insertAdjacentHTML(
        "beforeend",
    ...
    document.getElementById("whattheduck-civitai-mappings")?.addEventListener("click", (e) => {
      const target = e.target;
